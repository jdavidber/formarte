(function($){
			...
			frame: function() {
				...
				this._frame.on('update', function() {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=oxides_edge_gallery_upload_get_images&ids=" + ids,
	...
	function edgtfInitAjaxForm() {
		...
		$(document).delegate(".edgt_ajax_form", "submit", function (a) {
			var b = $(this);
			var c = {
				action: "oxides_edge_save_options"
			};
			jQuery.ajax({
				url: ajaxurl,
				cache: !1,
...
//            }
	...
	function edgtfShowHidePostFormats(){
		$('input[name="post_format"]').each(function(){
			$('#edgtf-meta-box-'+ $(this).attr("id")+'-meta').hide();
		});
		var $selected = $("input[name='post_format']:checked").attr("id");
		$('#edgtf-meta-box-'+ $selected+'-meta').fadeIn();
